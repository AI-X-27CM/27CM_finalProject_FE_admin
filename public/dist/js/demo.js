...
(function ($) {
  ...
  $(document).ready(function() {
    // 'navbar_all_colors' 배열이 정의되어 있는지 확인하세요.
    var active_navbar_color = null;
    var navbar_elements = $('.main-header');
  
    // 여기서 '.main-header' 요소가 존재하는지 확인하고 'active_navbar_color' 값을 설정합니다.
    if(navbar_elements.length > 0) {
      navbar_elements[0].classList.forEach(function (className) {
        if (navbar_all_colors.indexOf(className) > -1 && !active_navbar_color) {
          active_navbar_color = className.replace('navbar-', 'bg-');
        }
      });
  
      // 'active_navbar_color' 값이 설정되었는지 확인하고 설정되었다면 선택된 값을 업데이트합니다.
      if (active_navbar_color) {
        // active_navbar_color가 설정되어 있다면 이 부분이 실행됩니다.
        $navbar_variants_colors.find('option.' + active_navbar_color).prop('selected', true);
        $navbar_variants_colors.removeClass().addClass('custom-select mb-3 text-light border-0 ').addClass(active_navbar_color);
      } else {
        // active_navbar_color가 설정되어 있지 않다면 여기서 에러를 처리합니다.
        console.error('active_navbar_color is not defined');
      }
    } else {
      console.error('No elements with the class .main-header');
    }
  });
  ...
  // sidebar_colors 배열이 정의되어 있어야 합니다.
var sidebar_colors = ["bg-primary", "bg-warning", "bg-secondary","bg-success","bg-danger","bg-info","bg-light","bg-dark","bg-body","bg-white","bg-transparent",]; 
var active_sidebar_dark_color = null;
var mainSidebar = document.querySelector('.main-sidebar'); // main-sidebar 요소 선택

if (mainSidebar) {
  mainSidebar.classList.forEach(function (className) {
    // 필요한 코드 로직을 여기에 유지함
  });
}
